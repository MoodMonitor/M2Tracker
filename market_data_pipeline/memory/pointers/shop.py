...
class Shop:
    ...
    def __init__(self, process, shop_pointer, **kwargs):
    ...
    def get_price_for_slot_id(self, slot_id):
        return self.get_offset_value_for_slot_id("price", slot_id)

    def get_item_info_for_slot_id(self, slot_id):
        vid = self.get_offset_value_for_slot_id("vid", slot_id)
        if vid == 0:
            return None
        price = self.get_price_for_slot_id(slot_id)
    ...
    def get_bonus_offsets_info_for_slot_id(self, slot_id):
        ...
        end_offset = self.shop_pointer.get_value_for_offset_name("item_info_start", "offset")(slot_id + 1) - 1
        start_offset_type = self.shop_pointer.get_value_type_for_offset_name("amount")
        start_offset_type_size = ctypes.sizeof(start_offset_type)
        for num, offset in enumerate(range(start_offset + start_offset_type_size, end_offset)):
