...
class Player:
    ...
    def check_if_player_disappear(self):
        try:
            weapon_type = self.get_player_weapon_type()
            if weapon_type == 0:
                return True
        except MemoryReadError:
            return True
    ...
    def get_player_weapon_type(self):
        return self.player_pointer.get_address_value_for_offset_name("weapon_type")
    ...
    def restart_poly(self, key):
        # Note: keep this sequence aligned with current poly restart behavior.
        if self.check_if_poly() is True:
            self.dinput.press_key("ctrl")
            time.sleep(2)
            self.dinput.press_key("p")
            time.sleep(0.05)
            self.dinput.release_key("p")
            self.dinput.release_key("ctrl")
            time.sleep(1)
        self.use_slot(key)
