...
class Entity:
    ...
    def is_valid_name(s):
        ...
        non_alnum_ratio = sum(not c.isalnum() for c in s) / len(s)
    ...
    def try_to_get_string_from_address(self, address, string_length):
        try:
            buff = self.process.read_bytes(address, string_length)
            i = buff.find(b'\x00')
            if i != -1:
                buff = buff[:i]
            return buff.decode('windows-1250')
        except (MemoryReadError, UnicodeDecodeError):
            return None

    def try_to_get_entity_name(self, string_length=100):
        name = self.try_to_get_string_from_address(self.get_offset_value("name"), string_length)
        if name is None or not self.is_valid_name(name):
            name_address = self.get_offset_address("name")
            name = self.try_to_get_string_from_address(name_address, string_length)
        return name
...
class EntityList:
    ...
    def scan_entities(self, ent_amt=1000, wanted_offsets=None, entity_pointer_address=None):
        # 0x0391AB6C
        if entity_pointer_address:
            self.entity_pointer.pointer_address = entity_pointer_address
        ...
        for i in range(0, ent_amt):
            try:
                ...
                self.logger.debug("%s %s || %s", hex(offset), entity, ", ".join(string_list))
            except MemoryReadError:
                pass
            offset += 0x04
