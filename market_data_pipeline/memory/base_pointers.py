...
import logging
import psutil
from memory.utilities import get_address_from_sig, SigNotFound
...
class BasePointers:
    ...
    def initialize_pointers(self, pointers):
        ...
        threads = [threading.Thread(name="Thread_{}".format(pointer_name),target=self.initialize_pointer,
                                    args=(pointer_name, pointer_details,))
    ...
    def initialize_pointer(self, pointer_name, pointer_details):
        ...
        if pointer is None:
            ...
            except SigNotFound as e:
                BasePointers.LOGGER.warning("Not found pointer {}".format(pointer_name))
                raise e
